...
double settings_version = 1.12;


/*!
  These functions store and retrieve the program settings (environment) from a .ini file.  On both Linux and Windows
  the file will be called bagViewer.ini and will be stored in a directory called ABE.config.  On Linux, the
...
uint8_t envin (OPTIONS *options, QMainWindow *mainWindow)
{
...
#ifdef NVWIN3X
  QString ini_file = QString (getenv ("USERPROFILE")) + "/ABE.config/bagViewer.ini";
#else
  QString ini_file = QString (getenv ("HOME")) + "/ABE.config/bagViewer.ini";
#endif

  QSettings settings (ini_file, QSettings::IniFormat);
  settings.beginGroup ("bagViewer");
  ...
  options->color_option = settings.value (QString ("color by uncertainty flag"), options->color_option).toBool ();

  options->display_tracking_list = settings.value (QString ("display tracking list flag"), options->display_tracking_list).toBool ();

  options->display_tracking_info = settings.value (QString ("display tracking list info"), options->display_tracking_info).toBool ();

  options->feature_size = (float) settings.value (QString ("tracking list marker size"), (double) options->feature_size).toDouble ();

  options->input_dir = settings.value (QString ("input directory"), options->input_dir).toString ();
  options->image_dir = settings.value (QString ("image directory"), options->image_dir).toString ();
  ...
  red = settings.value (QString ("tracking list color/red"), options->feature_color.red ()).toInt ();
  green = settings.value (QString ("tracking list color/green"), options->feature_color.green ()).toInt ();
  blue = settings.value (QString ("tracking list color/blue"), options->feature_color.blue ()).toInt ();
  alpha = settings.value (QString ("tracking list color/alpha"), options->feature_color.alpha ()).toInt ();
  options->feature_color.setRgb (red, green, blue, alpha);


  red = settings.value (QString ("tracking list info color/red"), options->feature_info_color.red ()).toInt ();
  green = settings.value (QString ("tracking list info color/green"), options->feature_info_color.green ()).toInt ();
  blue = settings.value (QString ("tracking list info color/blue"), options->feature_info_color.blue ()).toInt ();
  alpha = settings.value (QString ("tracking list info color/alpha"), options->feature_info_color.alpha ()).toInt ();
...
void envout (OPTIONS *options, QMainWindow *mainWindow)
{
  ...
  settings.setValue (QString ("color by uncertainty flag"), options->color_option);

  settings.setValue (QString ("display tracking list flag"), options->display_tracking_list);

  settings.setValue (QString ("display tracking list info"), options->display_tracking_info);

  settings.setValue (QString ("tracking list marker size"), (double) options->feature_size);

  settings.setValue (QString ("input directory"), options->input_dir);
  settings.setValue (QString ("image directory"), options->image_dir);
  ...
  settings.setValue (QString ("tracking list color/red"), options->feature_color.red ());
  settings.setValue (QString ("tracking list color/green"), options->feature_color.green ());
  settings.setValue (QString ("tracking list color/blue"), options->feature_color.blue ());
  settings.setValue (QString ("tracking list color/alpha"), options->feature_color.alpha ());


  settings.setValue (QString ("tracking list info color/red"), options->feature_info_color.red ());
  settings.setValue (QString ("tracking list info color/green"), options->feature_info_color.green ());
  settings.setValue (QString ("tracking list info color/blue"), options->feature_info_color.blue ());
  settings.setValue (QString ("tracking list info color/alpha"), options->feature_info_color.alpha ());
